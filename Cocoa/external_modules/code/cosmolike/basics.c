...
double int_gsl_integrate_high_precision(double (*func)(double, void*), void* arg, double a, 
double b, double* error, int niter)
{
  double res, err;
  ...
  F.function = func;
  F.params = arg;
  gsl_integration_cquad(&F, a, b, 0, high, w, &res, &err, 0);
  if (NULL != error)
  {  
    *error = err;
  }
...
double int_gsl_integrate_medium_precision(double (*func)(double, void*), void* arg, double a, 
double b, double* error, int niter)
{
  double res, err;
  gsl_integration_cquad_workspace *w = gsl_integration_cquad_workspace_alloc(niter);
  ...
  F.function = func;
  F.params = arg;
  gsl_integration_cquad(&F, a, b, 0, medium, w, &res, &err, 0);
  if (NULL != error) {
    *error = err;
  }
  gsl_integration_cquad_workspace_free(w);
...
double int_gsl_integrate_low_precision(double (*func)(double, void*), void* arg, double a, double b, 
double* error __attribute__((unused)), int niter __attribute__((unused))) 
{
  ...
  F.params = arg;
  size_t neval;
  gsl_integration_qng(&F, a, b, 0, medium, &res, &err, &neval);
  return res;
}
...
double interpol(double* f, int n, double a, double b, double dx, double x, double lower,
double upper)
{
  if (x < a) 
  {
...
// like interpol, but f beeing a 2d-function 'lower' and 'upper' are the powers of a power 
// law extrapolation in the second argument	
...
double x, int ny, double ay, double by, double dy, double y, double lower) 
{
  ...
  else if (y > by) 
  {
    ...
    double upper = ((1. - dt) * (f[i][ny - 1] - f[i][ny - 1 - fitrange]) +
