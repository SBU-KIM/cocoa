...
void cfftlog_ells(double* x, double* fx, long N, config* config, int* ell, long Nell, double** y, 
double** Fy) 
{
  long N_original = N;
  long N_pad = config->N_pad;
  ...
  double eta_m[halfN + 1]; // Only calculate the m>=0 part
  ...
  if(N_extrap_high) 
  {
    ...
    {
      log_fatal("Can't log-extrapolate zero on the high side!");
      exit(1);
    }
    ...
    {
      log_fatal("Log-extrapolation on the high side fails due to sign change!");
      exit(1);
    }
  ...
  fftw_plan plan_forward = fftw_plan_dft_r2c_1d(N, fb, out, FFTW_ESTIMATE);
  fftw_execute(plan_forward);
  c_window_cfft(out, config->c_window_width, halfN);
  ...
  for (int j=0; j<Nell; j++) 
  {
    ...
    switch(config->derivative) 
    {
      case 0: 
      {
        g_l_cfft((double)ell[j], config->nu, eta_m, gl, halfN+1); 
        break;
      }
      case 1: 
      {
        g_l_1_cfft((double)ell[j], config->nu, eta_m, gl, halfN+1); 
        break;
      }
      case 2: 
      {
        g_l_2_cfft((double)ell[j], config->nu, eta_m, gl, halfN+1); 
        break;
      }
      default: 
      {
        log_fatal("Integral Not Supported! Please choose config->derivative from [0,1,2]");
        exit(1);
      }
    }
...
double** y, double** Fy) 
{
  ...
  for(int j=0; j<Nell; j++)
  {
    ...
    for(int i=0; i<N_original; i++) // calculate y arrays
    {
      y[j][i] = (ell[j]+1.) / x[N_original-1-i];
    }
    ...
    for(int i=0; i<N_original; i++) 
    {
      Fy[j][i] += out_ifft[j][i+N_pad+N_extrap_high]*sqrt(M_PI)/(4.0*N * pow(y[j][i], config->nu));
    }
  }
